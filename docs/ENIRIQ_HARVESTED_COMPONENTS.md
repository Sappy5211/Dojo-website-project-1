# ENIRIQ — Harvested Component Source (curated & cleaned)

> Companion to `ENIRIQ_BUILD_HANDOVER.md`. This file holds the **cleaned, un-escaped source for ONLY the harvested components the build keeps or repurposes** (handover §9A). It was extracted from the original `Animations code.md` and stripped of markdown escaping so you can copy it straight into files.
>
> **Deliberately excluded** (tied to banned proof — do **not** build them): `particle-text-effect`, `container-scroll-animation`, `gallery4`, `carousel`, `testimonials-columns-1`, `about-us-section`/`StatCounter`.
>
> **Aurora shader is not here** — use the production-hardened version in **handover §9.2** (it supersedes the raw block).

## Global rules for every block below
1. **Recolour** all demo colours → ENIRIQ palette tokens (handover §5.1): `energy #16A34A`, `energy-bright #10B981`, `cyan #38BDF8`, `ink/paper/mist`, amber rare.
2. **Strip every demo string and external URL** (`raw.githubusercontent.com`, `randomuser.me`, "shadcn/ui", author names, lorem). All copy/data comes from `content.ts`; all images are local in `/public`.
3. **`framer-motion` → `motion/react`** wherever it appears (APIs are identical). Never install framer-motion.
4. Keep/add `"use client"` on anything using hooks/motion/refs/browser APIs.
5. Honour `prefers-reduced-motion` (static fallback) and pre-size containers (no layout shift).

---

## 1. `spotlight-card` → `blocks/CapabilityCard.tsx` (+ `FocusAreaCard`) — KEEP
**Adapt:** pointer glow via `--x/--xp/--y/--yp` + `data-glow` (mechanism in handover §13.2). **Pin the hue to green→cyan** (`--base:140; --spread:50`, or fix `--hue:155`) so it never drifts to magenta. Render card content from `content.ts`; delete the demo. Use `glowColor="green"` semantics but constrained.
...
## 2. `tubelight-navbar` → merge into `nav/SiteNav.tsx` — KEEP (indicator only)
**Adapt:** **rewrite `framer-motion` → `motion/react`.** Use **only** the sliding "lamp" indicator and bind it to the **active route** via `usePathname()` — not to a standalone link list, not to scroll. Links come from `content.nav`. Ignore the `NavBarDemo` at the bottom.
...
## 3. `header-3` (`Header` + `MobileMenu`) → `nav/SiteNav.tsx` + `nav/MobileNav.tsx` — KEEP
**Adapt:** **rewrite `framer-motion` → `motion/react`.** `MenuToggleIcon` is an external dependency that is **not included** — replace it with a lucide `Menu`/`X` toggle. Drive active state from `usePathname()` (real routes). Keep the `navigation-menu` mega-menu (for What We Do / Sectors). **Cut** any scroll→hamburger morph or circular FAB pattern. Logo → ENIRIQ; links from `content.nav`; CTA "Talk to us" → `/contact`. Ignore any trailing demo.
...
And here’s a navigation button the 2 lines specifically how the animation for that should be
...
## 4. `team-section` → `blocks/PersonCard.tsx` + `/people` grid — KEEP
**Adapt:** **strip every `avatars.githubusercontent.com` URL** → local `/public/people` images or initials fallback (handover §13.7). Map to `content.people.leadership[]`. Add role/specialism/responsibility chip/LinkedIn (`ExternalLink`). Recolour.
...
## 5. `footer-section` → `blocks/Footer.tsx` — KEEP (strip)
**Adapt:** **remove the `Switch`/dark-toggle import and usage entirely.** **No Careers column.** Newsletter only if `flags.insightsEnabled`. Columns + details from `content.ts`. lucide `ExternalLink` for LinkedIn. Ignore the `Footerdemo` block at the bottom.
...
## 6. `radial-orbital-timeline` → `blocks/ValuesNetwork.tsx` — REPURPOSE (⚠ CONCEPTUAL DONOR ONLY)
**Do NOT ship this radial/orbital layout or its timer-based rotation.** It is included only so you can see the *mechanics* of node state + connecting lines + activation. **Rebuild** the Values signature as a connected **energy/data network** with **scroll-progress sequential activation** exactly per **handover §14** (not radial, not a clock, not space/SaaS). The donor build's `eniriq/src/components/sections/Approach.tsx` is the orbital version — reference its rAF/scroll mechanics, never its look.
...
*Curated from `Animations code.md`. Markdown escaping removed; demo content must still be stripped per the per-block notes. Primary doc: `ENIRIQ_BUILD_HANDOVER.md` (§9A maps these blocks).*
