...
function PeoplePhotoTile({
  ...
  return (
    <button
      type="button"
      className={cn(
        "group relative shrink-0 overflow-hidden rounded-2xl border border-ink-soft/10 bg-ink text-left shadow-[0_1.25rem_3rem_rgba(10,15,13,0.12)] transition-[opacity,transform,border-color] duration-300 hover:-translate-y-1 hover:border-energy/45 focus-visible:-translate-y-1",
        className,
...
function PeopleNameRow({
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 pl-7 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-ink-soft/48">{person.role}</p>
      <p className="mt-3 max-w-md pl-7 text-sm leading-relaxed text-ink-soft/64">{person.specialism}</p>
    </div>
  );
}
