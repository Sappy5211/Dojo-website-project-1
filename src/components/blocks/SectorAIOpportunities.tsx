      ...
      {items.map((item, i) => (
        ...
        >
          <GlowCard glowColor="cyan" className="flex h-full flex-col p-6">
            <h3 className="text-base font-semibold text-mist">{item.opportunity}</h3>
            <p className="mt-3 text-sm leading-relaxed text-mist/65">{item.dataNote}</p>
            ...
            <div className="mt-5 flex items-start gap-2 rounded-xl border border-energy/25 bg-energy/8 px-4 py-3">
              <span className="mt-0.5 flex-shrink-0 text-energy" aria-hidden>
              ...
              <p className="text-xs leading-relaxed text-energy/90">{item.oversightNote}</p>
            </div>
          </GlowCard>
        </motion.div>
      ))}
